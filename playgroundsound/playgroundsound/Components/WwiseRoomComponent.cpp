...
void WwiseRoomComponent::InitRoom(GameObject* in_gameObject) {
    ...
    for (const auto& [point0, point1, point2] : triangles) {
        akTriangles.emplace_back(
            static_cast<AkVertIdx>(point0),
            static_cast<AkVertIdx>(point1),
            static_cast<AkVertIdx>(point2),
            static_cast<AkSurfIdx>(0));
    }
...
void WwiseRoomComponent::InitRoomGeometry(GameObject* roomObj) {
    ...
    for (size_t i = 0; i < trianglesData.size(); ++i) {
        akTriangles[i] = {
            static_cast<AkVertIdx>(trianglesData[i].point0),
            static_cast<AkVertIdx>(trianglesData[i].point1),
            static_cast<AkVertIdx>(trianglesData[i].point2),
            static_cast<AkSurfIdx>(0)
        };
    }
...
void WwiseRoomComponent::GenerateWalls(const GameObject* gameObject, const uint64_t& roomID,
    ...
    for (size_t i = 0; i < 6; i++) {
        ...
        if (i == 0) {
            ...
            position = {
                gameObject->m_transform.position.x, gameObject->m_transform.position.y,
                gameObject->m_transform.position.z + 5
            };
            frontVector = {1, 0, 0};
        }
        else if (i == 1) {
            ...
            akGeometrySetID = wallSidesGeometryID;
            position = {
                gameObject->m_transform.position.x + 5, gameObject->m_transform.position.y,
                gameObject->m_transform.position.z
            };
            frontVector = {0, 0, 1};
        }
        else if (i == 3) {
            ...
            position = {
                gameObject->m_transform.position.x, gameObject->m_transform.position.y + 5,
                gameObject->m_transform.position.z
            };
            frontVector = {1, 0, 0};
            topVector = {0, 0, 1};
...
void WwiseRoomComponent::SetBoundingBox(const GO_Vector3& center, const float& width, const float& length,
                                        const float& height) {
    boundingBox = {center, width, length, height};
}

const GO_BoundingBox& WwiseRoomComponent::GetBoundingBox() const {
    return boundingBox;
}

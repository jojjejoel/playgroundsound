...
void RenderComponent::Draw(const GameObject* in_gameObject) const {
    const Vector3 pos = {
        in_gameObject->m_transform.position.x, in_gameObject->m_transform.position.y,
        in_gameObject->m_transform.position.z
    };
    const Vector3 scale = {
        in_gameObject->m_transform.scale.x, in_gameObject->m_transform.scale.y, in_gameObject->m_transform.scale.z
    };
    const Color drawColor = {
        static_cast<unsigned char>(color.x), static_cast<unsigned char>(color.y), static_cast<unsigned char>(color.z),
        255
    };
    if (shouldRender) {
...
void RenderComponent::SetModel(Model* in_model, const bool& in_shouldRender, const bool& initRoom,
                               const GO_Vector3& in_color) {
    model = in_model;
...
void RenderComponent::InitRoomGeometry() {
    for (size_t i = 0, v = 0; std::cmp_less(i, model->meshes[0].vertexCount); i++, v += 3) {
        GoVertex vertex;
        const float* vertices = model->meshes[0].vertices;
        Vector3 vector3;
    ...
    for (size_t i = 0, v = 0; std::cmp_less(i, model->meshes[0].triangleCount); i++, v += 3) {
        Triangle triangle;
...
void RenderComponent::SetShouldRender(const bool& in_shouldRender) {
    shouldRender = in_shouldRender;
}
